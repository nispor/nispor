...
UPSTERAM_GIT="https://github.com/nispor/nispor.git"
NEW_RELEASE_URL="https://github.com/nispor/nispor/releases/new"
...
cd $CODE_BASE_DIR/src/lib
cargo publish
cd $CODE_BASE_DIR/src/cli
cargo publish

cd $CODE_BASE_DIR
make dist
mv -v nispor-$CUR_VERSION.tar.gz nispor-vendor-$CUR_VERSION.tar.xz /tmp/
echo "Please upload these nispor tarballs in /tmp folder"
...
sed -i -e "s/$CUR_VERSION/$NEXT_VERSION/" \
    VERSION src/*/Cargo.toml src/python/setup.py
