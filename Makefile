...
CLIB_SO_MAN=libnispor.so.0
CLIB_SO_MIN=libnispor.0.2
CLIB_SO_FULL=libnispor.0.2.1
CLIB_SO_DEV_RELEASE=$(RUST_RELEASE_BIN_DIR)/$(CLIB_SO_DEV)
PYTHON_MODULE=nispor.py
PYTHON_MODULE_RELEASE=src/python/$(PYTHON_MODULE)
...
$(CLI_EXEC_RELEASE) $(VARLINK_SRV_EXEC_RELEASE) $(PYTHON_MODULE):
...
install: $(VARLINK_SRV_EXEC_RELEASE) $(CLI_EXEC_RELEASE) $(PYTHON_MODULE)
	...
	install -v -D -m755 $(PYTHON_MODULE_RELEASE) \
		$(DESTDIR)$(PYTHON3_SITE_DIR)/$(PYTHON_MODULE)
...
uninstall:
	...
	rm -fv $(DESTDIR)$(PYTHON3_SITE_DIR)/nispor.py
