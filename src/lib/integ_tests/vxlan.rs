...
const VXLAN_CHANGE_YAML: &str = r#"
interfaces:
  - name: vxlan1
    type: vxlan
    mtu: 1290
    mac-address: c6:6a:ee:d1:a1:1e
    vxlan:
      base-iface: dummy2
      local: 2.2.2.2
      ttl: 32
...
const CHANGED_VXLAN_INFO: &str = r#"---
remote: 8.8.8.8
vxlan-id: 102
base-iface: dummy2
local: 2.2.2.2
...
#[test]
fn test_change_existing_vxlan() {
    with_vxlan_conf(|| {
        let net_conf: NetConf =
            serde_yaml::from_str(VXLAN_CHANGE_YAML).unwrap();
        net_conf.apply().unwrap();
        let state = NetState::retrieve().unwrap();
        let iface = &state.ifaces[IFACE_NAME];
        assert_eq!(iface.iface_type, crate::IfaceType::Vxlan);
        assert_eq!(iface.mtu, 1290);
        assert_eq!(iface.mac_address, "c6:6a:ee:d1:a1:1e");
        assert_value_match(CHANGED_VXLAN_INFO, &iface.vxlan);
    });
}
