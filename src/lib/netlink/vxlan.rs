...
pub(crate) fn parse_vxlan_info(data: &[u8]) -> VxlanInfo {
    ...
    while i < data.len() {
        ...
        match hdr.nla_type {
            IFLA_VXLAN_ID => {
                info.vxlan_id = parse_as_u32(data);
            }
            IFLA_VXLAN_GROUP => {
                info.remote = parse_as_ipv4(data);
            }
            IFLA_VXLAN_LINK => {
                info.base_iface = format!("{}", parse_as_u32(data));
            }
            IFLA_VXLAN_LOCAL => {
                info.local = parse_as_ipv4(data);
            }
            IFLA_VXLAN_TTL => {
                info.ttl = parse_as_u8(data);
            }
            IFLA_VXLAN_TOS => {
                info.tos = parse_as_u8(data);
            }
            IFLA_VXLAN_LEARNING => {
                info.learning = parse_as_u8(data) > 0;
            }
            IFLA_VXLAN_AGEING => {
                info.ageing = parse_as_u32(data);
            }
            IFLA_VXLAN_LIMIT => {
                info.max_address = parse_as_u32(data);
            }
            IFLA_VXLAN_PORT_RANGE => {
                info.src_port_min = parse_as_be16(data);
                info.src_port_max = parse_as_be16(&[data[2], data[3]]);
            }
            IFLA_VXLAN_PROXY => {
                info.proxy = parse_as_u8(data) > 0;
            }
            IFLA_VXLAN_RSC => {
                info.rsc = parse_as_u8(data) > 0;
            }
            IFLA_VXLAN_L2MISS => {
                info.l2miss = parse_as_u8(data) > 0;
            }
            IFLA_VXLAN_L3MISS => {
                info.l3miss = parse_as_u8(data) > 0;
            }
            IFLA_VXLAN_PORT => {
                info.dst_port_min = parse_as_be16(data);
                info.dst_port_max = parse_as_be16(&[data[2], data[3]]);
            }
            IFLA_VXLAN_GROUP6 => {
                info.remote = parse_as_ipv6(data);
            }
            IFLA_VXLAN_LOCAL6 => {
                info.local = parse_as_ipv6(data);
            }
            IFLA_VXLAN_UDP_CSUM => {
                info.udp_check_sum = parse_as_u8(data) > 0;
            }
            IFLA_VXLAN_UDP_ZERO_CSUM6_TX => {
                info.udp6_zero_check_sum_tx = parse_as_u8(data) > 0;
            }
            IFLA_VXLAN_UDP_ZERO_CSUM6_RX => {
                info.udp6_zero_check_sum_rx = parse_as_u8(data) > 0;
            }
            IFLA_VXLAN_REMCSUM_TX => {
                info.remote_check_sum_tx = parse_as_u8(data) > 0;
            }
            IFLA_VXLAN_REMCSUM_RX => {
                info.remote_check_sum_rx = parse_as_u8(data) > 0;
            }
            IFLA_VXLAN_GBP => {
                info.gbp = true;
            }
            ...
            IFLA_VXLAN_COLLECT_METADATA => {
                info.collect_metadata = parse_as_u8(data) > 0;
            }
            IFLA_VXLAN_LABEL => {
                info.label = parse_as_be32(data);
            }
            IFLA_VXLAN_GPE => {
                info.gpe = true;
            }
            IFLA_VXLAN_TTL_INHERIT => {
                info.ttl_inherit = parse_as_u8(data) > 0;
            }
            IFLA_VXLAN_DF => {
                info.df = parse_as_u8(data);
            }
            _ => eprintln!("Unknwon nla_type {}, {:?}", hdr.nla_type, data),
        }
        i = i + hdr.nla_len;
    }
    info
